...
public abstract class JBoard extends JComponent {
	...
	private void drawCell(int row, int col, Graphics g) {
		...
		if (p != null) {
			...
			case RECTANGLE:
				...
				break;
			case CIRCLECHESS:
				nombreFicha = setNombreFicha(row, col);
				ImageIcon img = new ImageIcon(Utils.loadImage(nombreFicha));
				g.fillOval(x + _SEPARATOR+2, y + _SEPARATOR+2, _CELL_WIDTH - 2*_SEPARATOR-4, _CELL_HEIGHT - 2*_SEPARATOR-4);
				g.setColor(Color.black);
				g.drawOval(x + _SEPARATOR+2, y + _SEPARATOR+2, _CELL_WIDTH - 2*_SEPARATOR-4, _CELL_HEIGHT - 2*_SEPARATOR-4);
				g.drawImage(img.getImage(),x + _SEPARATOR+2, y + _SEPARATOR+2, _CELL_WIDTH - 2*_SEPARATOR-4, _CELL_HEIGHT - 2*_SEPARATOR-4, this);				
				break;
	...
	protected abstract String setNombreFicha(int row, int col);
